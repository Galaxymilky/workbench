...
public class EncryptUtil {
    private static byte[] DecryptData(Cipher cipher, byte cipherText[], String sKey) throws Exception {
        java.security.Key key = new SecretKeySpec(sKey.getBytes(), "DES");
        java.security.spec.AlgorithmParameterSpec paramSpec = new IvParameterSpec(sKey.getBytes());
    ...
    public static String DecryptPassword(String decryptString) {
        ...
        try {
            ...
            byte cleartext[] = DecryptData(cipher, decryptByte, "mailpass");
    ...
    private static byte[] EncryptData(Cipher cipher, byte plainText[], String sKey) throws Exception {
        java.security.Key key = new SecretKeySpec(sKey.getBytes(), "DES");
        java.security.spec.AlgorithmParameterSpec paramSpec = new IvParameterSpec(
                sKey.getBytes());
    ...
    public static String EncryptPassword(String encryptString) {
        ...
        try {
            ...
            Cipher cipher = Cipher.getInstance("DES/CBC/PKCS5Padding");
            byte ciphertext[] = EncryptData(cipher, encryptString
                    .getBytes(), "mailpass");
