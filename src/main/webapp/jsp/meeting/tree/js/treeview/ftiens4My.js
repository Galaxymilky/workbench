...
// ***************************************************************** 
function MyFolder(folderDescription, hreference) //constructor 
{ 
  //constant data 
  ...
  //methods 
  this.initialize = MyinitializeFolder 
  this.setState = MysetStateFolder 
  this.MyaddChild = MyaddChild 
  this.addChildren = addChildren
  this.createIndex = MycreateEntryIndex 
  this.escondeBlock = escondeBlock
  this.esconde = escondeFolder 
  this.folderMstr = folderMstr 
  this.renderOb = MydrawFolder 
  this.totalHeight = totalHeight 
  this.subEntries = MyfolderSubEntries 
...
function MyinitializeFolder(level, lastNode, leftSide) 
{ 
  var i=0       
  nc = this.nChildren 
  ...
  { 
    ...
    for (i=0 ; i < this.nChildren; i++)  
    {
      if (typeof this.children[i].initialize == 'undefined') //document node was specified using the addChildren function
      {
        ...
        this.children[i].initialize=MyinitializeItem;
        this.children[i].createIndex=MycreateEntryIndex;
...
function MydrawFolder(insertAtObj) 
{ 
  var nodeName = ""
  var auxEv = ""
  var docW = ""
  ...
  if (browserVersion > 0) 
    auxEv = "<a href='javascript:MyclickOnNode(\""+this.getID()+"\")'>" 
  else 
    auxEv = "<a>" 
...
function MysetStateFolder(MyisOpen) 
{ 
  var subEntries 
  var totalHeight 
  var fIt = 0 
  ...
  { 
    ...
    for (fIt = this.id + subEntries + 1; fIt < MynEntries; fIt++) 
      MyindexOfEntries[fIt].navObj.moveBy(0, totalHeight) 
  }  
  this.MyisOpen = MyisOpen;

  if (this.getID()!=foldersTreeMy.getID() && PRESERVESTATE && !this.MyisOpen) //closing
  {
  ...
  if (!this.MyisOpen && this.isLastOpenedfolder)
  {
		lastOpenedFolder = null;
		this.isLastOpenedfolder = false;
  }
  MypropagateChangesInState(this) 
} 
 
function MypropagateChangesInState(folder) 
{   
  var i=0 
...
function MyItem(itemDescription) // Constructor 
{ 
  // constant data 
  ...
  this.initialize = MyinitializeItem;
  this.createIndex = MycreateEntryIndex;
  this.forceOpeningOfAncestorFolders = forceOpeningOfAncestorFolders;
...
function finalizeCreationOfItem(itemArray)
{
  ...
  itemArray.iconSrc = ICONPATH + "ftv2leaf.gif" 
...
function MyinitializeItem(level, lastNode, leftSide) 
{  
  this.createIndex() 
  this.level = level
  this.leftSideCoded = leftSide
...
function MydrawItem(insertAtObj) 
{ 
  var leftSide = leftSideHTML(this.leftSideCoded)
  var docW = ""
  ...
  if (this.level>0) 
    ...
    { 
      leftSide = leftSide + "<td valign=top><img src='" + this.nodeImageSrc() + "' width=16 height=22></td>"
    } 
    else 
    { 
...
function forceOpeningOfAncestorFolders() {
  if (this.parentObj == null || this.parentObj.MyisOpen)
    return
  else {
...
function MycreateEntryIndex() 
{ 
  this.id = MynEntries 
  MyindexOfEntries[MynEntries] = this 
  MynEntries++ 
} 
...
{ 
  ...
  if (this.MyisOpen) //is a folder and _is_ open 
    for (i=0 ; i < this.nChildren; i++)  
...
{ 
  ...
  {
    alert("Treeview was not able to find the node object corresponding to ID=" + folderId + ". If the configuration file sets a.xID values, it must set them for ALL nodes, including the foldersTreeMy root.")
    return;
  }
  ...
  if (lastOpenedFolder != null && lastOpenedFolder != folderId)
    MyclickOnNode(lastOpenedFolder); //sets lastOpenedFolder to null
...
function MyclickOnNode(folderId) 
{ 
  fOb = findObj(folderId);
  if (typeof fOb=='undefined' || fOb==null)
  {
    alert("Treeview was not able to find the node object corresponding to ID=" + folderId + ". If the configuration file sets a.xID, it must set foldersTreeMy.xID as well.")
    return;
  }
...
{ 
  ...
  state = folderObj.MyisOpen 
  folderObj.setState(!state) //open<->close  

  if (folderObj.id!=foldersTreeMy.id && PRESERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
    if (currentOpen == null)
      currentOpen = ""

    if (!folderObj.MyisOpen) //closing
    {
...
function findObj(id)
{
  ...
  if (typeof foldersTreeMy.xID != "undefined") {
    nodeObj = MyindexOfEntries[i];
    for(i=0;i<MynEntries&&MyindexOfEntries[i].xID!=id;i++) //may need optimization
      ;
    id = i
  }
  if (id >= MynEntries)
...
function MyinsFld(parentFolder, childFolder) 
{ 
  return parentFolder.MyaddChild(childFolder) 
} 
 
function MyinsDoc(parentFolder, document) 
{ 
  return parentFolder.MyaddChild(document) 
} 

function MygFld(description, hreference) 
{ 
  folder = new MyFolder(description, hreference);
  return folder;
} 
 
function MygLnk(optionFlags, description, linkData) 
{ 
  if (optionFlags>=0) { //is numeric (old style) or empty (error)
    ...
    alert("Change your Treeview configuration file to use the new style of target argument in MygLnk");
    return;
  }

  newItem = new MyItem(description);
...
function storeAllNodesInClickCookie(treeNodeObj)
{
  ...
  if (typeof treeNodeObj.setState != "undefined") //is folder
  {
    ...
    if (treeNodeObj.getID() != foldersTreeMy.getID())
...
function CookieBranding(name) {
  if (typeof foldersTreeMy.treeID != "undefined")
    return name+foldersTreeMy.treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
var MyindexOfEntries = new Array 
var MynEntries = 1000;// cuijie for tree.js
...
{ 
  ...
  //foldersTreeMy (with the site's data) is created in an external .js (demoFramesetNode.js, for example)
  foldersTreeMy.initialize(0, true, "") 
  if (supportsDeferral && !STARTALLOPEN) {
      foldersTreeMy.renderOb(null) //delay construction of nodes
  }

  else {
    renderAllTree(foldersTreeMy, null);

    if (PRESERVESTATE && STARTALLOPEN)
      storeAllNodesInClickCookie(foldersTreeMy)

    //To force the scrollable area to be big enough
    if (browserVersion == 2) 
      doc.write("<layer top=" + MyindexOfEntries[MynEntries-1].navObj.top + ">&nbsp;</layer>") 

    if (browserVersion != 0 && !STARTALLOPEN)
      hideWholeTree(foldersTreeMy, false, 0)
  }
